...
class PDMTrafficScorer(PDMScorer):
    ...
    def extract_centerline_for_agent(
        ...
        nearby_lanes = [
            map_obj
            for map_objects in proximal_objects.values()
        ...
        if len(nearby_lanes) == 0:
            ...
            agent_point = Point(
                agent_states[0][StateIndex.X], agent_states[0][StateIndex.Y]
            )
            dist_to_lanes = [
                lane.polygon.distance(agent_point) for lane in nearby_lanes
            ]
        ...
        for agent_state in agent_states[1:]:
            ...
            dist_to_lanes = [
                lane.polygon.distance(agent_point) for lane in candidate_lanes
            ]
        ...
        agent_centric_detection_tracks = [
            DetectionsTracks(
                tracked_objects=TrackedObjects([
                    o for o in tracks.tracked_objects
                    if o.track_token != agent_token
                ] + [ego_tracked_objects[t]])
            )
    ...
    def score_proposals(
        ...
        map_api = get_maps_api(
            map_parameters.map_root, map_parameters.map_version, map_parameters.map_name
        )
        ...
        simulated_agents = {
            ...
            for (token, simulated_agent_trajectory, simulated_agent_mask) 
            in zip(simulated_agent_tokens, simulated_agent_trajectories, simulated_agent_masks)
        }
        logreplay_agents = {
            token: (logreplay_agent_trajectory, logreplay_agent_mask)
            for (token, logreplay_agent_trajectory, logreplay_agent_mask)
            in zip(logreplay_agent_tokens, logreplay_agent_trajectories, logreplay_agent_masks)
        }
        ...
        results = [
            pd.concat([ego_score, agent_scores], axis=1)
            for ego_score in ego_scores
        ]
